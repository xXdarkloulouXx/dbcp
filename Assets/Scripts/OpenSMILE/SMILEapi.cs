...
using System.Text;
using AOT; // Nécessaire pour [MonoPInvokeCallback]
...
namespace AudEERING.openSMILE
{
    ...
    public class OpenSMILE : IDisposable
    {
        ...
        // Dictionnaire statique pour mapper l'IntPtr vers l'objet C#
        private static Dictionary<IntPtr, OpenSMILE> instances = new Dictionary<IntPtr, OpenSMILE>();

        // Les délégués sont maintenant statiques
        private static LogCallback logCallback = OnLogMessage;
        private static StateChangedCallback stateCallback = OnStateChanged;
        ...
        public void Initialize(string configFile, Dictionary<string, string> options, int logLevel=2, bool debug=false, bool consoleOutput=false)
        {
            ...
            // Ajoute cet objet au dictionnaire statique
            lock (instances)
            {
                instances[smileObj] = this;
            }
        ...
        private static void OnLogMessage(IntPtr smileobj, OpenSmileLogMessage message, IntPtr param)
        {
            OpenSMILE instance;
            lock (instances)
            {
                if (!instances.TryGetValue(smileobj, out instance))
                    return; 
            }
            instance.LogMessage?.Invoke(instance, new LogMessageEventArgs(message));
        }

        [MonoPInvokeCallback(typeof(StateChangedCallback))]
        private static void OnStateChanged(IntPtr smileobj, OpenSmileState state, IntPtr param)
        {
            OpenSMILE instance;
            lock (instances)
            {
            ...
            instance.StateChanged?.Invoke(instance, new StateChangedEventArgs(state));
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            // Supprime l'objet du dictionnaire statique
            lock (instances)
            {
                if (smileObj != IntPtr.Zero)
                {
                    instances.Remove(smileObj);
                }
            }
...
} // <-- CORRECTION 2 : Suppression de l'accolade '}' supplémentaire qui était ici